...
/**
 * Patch the VM's package.json to remove --turbopack from the dev script.
 * Turbopack fails in the Freestyle sandbox because it cannot resolve the
 * Next.js workspace root from the /workspace directory.
 */
const ensureVmReady = async (vm: Vm): Promise<void> => {
  try {
    ...
    if (!devScript.includes("--turbopack")) {
      // Already patched or doesn't use turbopack – nothing to do
      return;
    }
    ...
    // Restart the dev server so the new script takes effect
    const execResult = await (vm as unknown as { exec: (opts: { command: string }) => Promise<unknown> }).exec({
      command: `cd ${WORKDIR} && pkill -f "next dev" || true && sleep 1 && nohup npm run dev > /tmp/next-dev.log 2>&1 &`,
    });
    console.log("[v0] VM package.json patched, dev server restarted:", execResult);
  } catch (error) {
    // Non-fatal: the LLM may still be able to work even if patching fails
    console.error("[v0] ensureVmReady failed (non-fatal):", error);
  }
};
